...
(function (angular) {
    ...
    angular.module("ngUtils.baseServices", ["ngResource"])
        ...
        .constant("app", {
             ...
             * 指令的模板根目录
             */
            componentsDir: window.ResourceDir + "/common/components/",
        ...
        .factory("defaultRestCache", ["$cacheFactory", function ($cacheFactory) {
            ...
            return $cacheFactory("defaultRestCache", {
                capacity: 50
            });
        ...
        // 配置restful接口前缀
        .constant("RESOURCE_PREFIX", "/rest/1.0/")

        // 生成resource
        .factory("genResource", ["$resource", "defaultRestCache", "RESOURCE_PREFIX", function ($resource, defaultRestCache, RESOURCE_PREFIX) {
            ...
            return function (url, cache, params, additionalActions) {
                ...
                return $resource(RESOURCE_PREFIX + url, params, angular.extend(DEFAULT_ACTIONS, additionalActions));
            };
        }])
